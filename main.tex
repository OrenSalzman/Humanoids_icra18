...
The second task we considered was mounting onto a ladder from a standing position (see Fig.~\ref{fig:gui}) to the desired contact poses. This again is a challenging planning problem because the ladder rungs and the robot grasping hooks construct narrow spaces in the configuration space which have to be traversed to be able to establish the desired contacts. The baseline heuristic function that we used in this experiment is computed as the summation of four six-dimensional Euclidean distances between each of the four end effectors and their respective target poses. 
...
%\captionsetup{format = plain}
  ...
  {
  \includegraphics[width=0.37\textwidth]{fig/ed_plot.pdf}
  \label{fig:ed_plot}
...
%
  ...
  {
  \includegraphics[width=0.37\textwidth]{fig/h_plot.pdf}
  \label{fig:h_plot}
...
\begin{table}
  \begin{tabular}{lcccc}
    ...
    \multirow{2}{*}{Dataset} &
      ...
      & (a) & (b) & (a) & (b) \\
      \hline
    Planning time(s) & 205 & 183 & - & - \\
    Total time(s) & 293 & 257 & - & - \\
    Expansions & 1800 & 1770 & - & - \\
    Num. of guidances & 5.4 & 5.3 & - & - \\
    Guidance time & 16.4 & 13.6 & - & - \\
    \hline
  \end{tabular}
\caption{Experimental results for the bipedal locomotion and ladder mounting tasks averaged over 10 trials}\label{tab:stats}
\label{tab:stats}
\end{table}

...
\os{should we use subscript $\Q$?}

\os{Definitions for local minima:
(i)~stagnation region,
(ii)~stalled progress,
(iii)~local minima
(iv)~depression
...
\begin{figure}[tb]
  ...
  \caption{
		...
		\os{should we introduce the notions of baseline and dynamic heuristics formally in text?}
		}
\vspace{-5mm}
   	\label{fig:res}
\end{figure}
...
\label{sec:future}
\os{add discussion on sampling based planners}

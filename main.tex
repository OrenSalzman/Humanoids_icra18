...
\begin{figure}[tb]
  \centering
  	\includegraphics[width=0.5\textwidth]{fig/cover3.png}
  	%\vspace{-2mm}
  \caption{
...
In such settings, guidance ranged from teleoperating the robot through shared autonomy to high-level 
...
\label{sec:mha}
Multi Heuristic \astar (\mhastar)~\cite{ASNHL16,NAL15} is a search-based planning algorithm that takes in multiple, possibly inadmissible heuristic functions in addition to a single consistent heuristic termed the \emph{anchor} heuristic.
...
Part of the efficiency of \mhastar is due to the fact that the value of the cost-to-come (the $g$-value) computed for each state is shared between all the different searches\footnote{To be precise, Aine et al.~\cite{ASNHL16} define two variants of \mhastar: Independent and Shared \mhastar where the queues do not share and do share the $g$-values of states, respectively. In this paper when we use the term \mhastar, it refers to the latter (shared) variant.}.
...
As we will see, we will use a slightly different characterization of when the planner ceases to make progress which we call \emph{stagnation regions}. For details, see Sec.~\ref{}. 
}
...
We will use the notion of expansion delays to identify when the planner is in a stagnation region (i.e., when it ceases to make progress towards the goal). For details, see Sec.~\ref{}.
}
...
We demonstrate our general planning framework described in Sec.~\ref{sec:high} for the case where the motion-planning algorithm~$\A$ is multi-heuristic \astar (\mhastar)~\cite{ASNHL16}.
...
\begin{figure}
  \centering
  \includegraphics[width=0.475\textwidth]{fig/local_min_detection_new2.pdf}
  \caption{%
...
%
We identify when the planner is in a stagnation region using the notion of expansion delays~\cite{DTR11}:
...
\os{
Given some parameter $\omega >0$ we compute $\Delta e(\omega)$---the average expansion delay over a moving window of size $\omega$. This gives a principled way to identify when a planner ceases to make progress towards the goal, a state which we call \emph{stagnation}:
\begin{definition}
Let 
...
\begin{figure}[tb]
  \centering
  	\includegraphics[width=0.453\textwidth]{fig/panel.png}
  	%\vspace{-2mm}
  \caption{
...
%\captionsetup{format = plain}
  ...
  \subfigure[]
  {
  \includegraphics[width=0.2125\textwidth]{fig/example_1.pdf}
  \label{fig:dynamic_heuristic1}
  }
  \subfigure[]
  {
  \label{fig:dynamic_heuristic2}
  \includegraphics[width=0.2125\textwidth]{fig/example_2.pdf}
  }
  \subfigure[]
  {
  \label{fig:dynamic_heuristic3}
  \includegraphics[width=0.2125\textwidth]{fig/example_3.pdf}
  }  
  \subfigure[]
  {
  \label{fig:dynamic_heuristic4}
  \includegraphics[width=0.2125\textwidth]{fig/example_4.pdf}
  }
  \caption{%
    ...
    %
    In this example \mhastar alternates between queues in a round-robin fashion. Furthermore, heuristics are inflated by a weight of $w=\infty$. Namely, the priority queues are ordered according to the heuristic-value of the states.
    %
...
Equipped with the heuristic $\hat{h}$, we add a new queue to the \mhastar algorithm prioritized using the heuristic $\hat{h}$. %(Alg~\ref{alg:main}, line~4). 
...
Note that in \mhastar, nodes are shared between the different queues.
...
We are now ready to describe how we apply our general framework of user-guided planning to the case of \mhastar.
...
In one of the experiments (Sec.~\ref{subsec:locomotion}), we employ the recently-introduced adaptive dimensionality framework~\cite{GCBSL11,GSL12,GSL13}.
At a high level,  it consists of two stages: an adaptive planning phase which attempts to plan in a low-dimensional space when possible and a tracking phase which plans in the high-dimensional space.
...
To construct the graph on the fly, we used a set of motion primitives which are short kinematically feasible motion sequences to generate the actions, or the successors, of a state during the search. These primitives generate motions for the limbs and torso of the robot (cite Andrew's paper). 
...
The parameter values that we used consistently in the two experiments are $\omega_1 (200), \omega_2 (50)$ and $\varepsilon (50)$ for the heuristic-based, and $\tau (30)$ and $\omega (10)$ for the vacillation-based stagnation region detection.
Finally in our final experiment, we show that while these approaches have a number of parameters, it is highly robust to the choice of parameters for our domain.

\subsection{Bipedal locomotion}
\label{subsec:locomotion}
The first task we considered is bipedal locomotion of the robot while avoiding collisions with the obstacles in the environment as well as adhering to the physical constraints of the robot. The initial state of the robot is the full dimensional 34-DOF configuration whereas the goal is represented as a cylindrical region which the robot needs to enter (depicted in Fig.~\ref{fig:robot}).

As explained earlier, for this experiment we used the adaptive-planning framework. The baseline heuristic is desined to assist the search with a general walking or stepping capability. For each step the footprint of the target footstep is visualized for the user to be able to better understand where the planner is stuck and how to provide the guidance. In relatively easier scenarios this heuristic would suffice in guiding the search. However if the search encounters harder situations such as those depicted in Fig.~\ref{fig:biped_locomotion}, then it is likely to get trapped into a stagnation region and would invoke user-guidance.

Results demonstrating the effectiveness of our framework are depicted in Fig.~\ref{fig:stair_plot}
...
The second task we considered was mounting onto a ladder from a standing position (see Fig.~\ref{fig:gui}) to the desired contact poses. This again is a challenging planning problem because the ladder rungs and the robot grasping hooks construct narrow spaces in the configuration space which have to be traversed to be able to establish the desired contacts. The baseline heuristic function that we used in this experiment is computed as the summation of four six-dimensional Euclidean distances between each of the four end effectors and their respective target poses. The results for this experiment are depicted in Fig.~\ref{fig:stair_plot}
...
%\captionsetup{format = plain}
  ...
  \subfigure[]
  {
  \includegraphics[width=0.225\textwidth]{fig/bar_2.png}
  \label{fig:stuck_bar}
  }
  \subfigure[]
  {
  \label{fig:stuck_tables}
  \includegraphics[width=0.225\textwidth]{fig/tables_1.png}
  }
  \subfigure[]
  {
  \label{fig:stuck_door}
  \includegraphics[width=0.225\textwidth]{fig/door_2.png}
  }  
  \subfigure[]
  {
  \label{fig:stuck_step}
  \includegraphics[width=0.225\textwidth]{fig/step_2.png}
  }
  \caption{%
    Biped locomotion in challenging scenarios: 
  \subref{fig:stuck_bar}~shows an instance where the robot has to squat or bend down to pass under a beam.
    %
  \subref{fig:stuck_tables}~depicts a situation where the robot has to pass through two tables by may be lifting the elbows high above the tables.
    %
  Progressing further, \subref{fig:stuck_door}~is a situation where the robot has to squeeze through narrow doorway by tucking in its arms.
    %
  \subref{fig:stuck_step}~shows a scenario where the robot has to step onto a relatively high platform to reach the goal for which it has to lean a little bit more to it's right side to be able to lift the left foot further up.
  }%
...
%
  ...
  \caption{%
    Progress of \mhastar with and without user guidance---heuristic values as a function of the number of queue expansions.

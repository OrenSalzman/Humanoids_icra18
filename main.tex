...
\begin{figure}[tb]
  \centering
  	\includegraphics[width=0.4\textwidth]{fig/cover3.png}
  	%\vspace{-2mm}
  \caption{
...
While our approach is general and can be incorporated with any motion-planning algorithm 
...
%
We suggest two alternative methods to identify when the planner ceases to make progress toewards the gaol, each resulting in a slightly different definition of a  stagnation region.
...
Thus, we suggest an alternative method, which we call \emph{heuristic-base}, to detect when the planner ceases to progress towards the goal.
...
In one of the experiments (Sec.~\ref{subsec:locomotion}), we employ the recently-introduced adaptive dimensionality 
\arxiv{framework~\cite{GCBSL11,GSL12,GSL13}.}{framework~\cite{GCBSL11,GSL12}.}
...
To construct the graph on the fly, we used a set of motion primitives which are short kinematically feasible motion sequences to generate the actions, or the successors, of a state during the search. These primitives generate motions for the limbs and torso of the robot (\todo{cite Andrew's paper}). 
...
%\captionsetup{format = plain}
  ...
  \subref{fig:stuck_bar}~an instance where the robot has to squat or bend down to pass under a beam.
    %
  ...
  \subref{fig:stuck_door}~the robot has to squeeze through a narrow doorway by tucking in its arms.
    %
  \subref{fig:stuck_step}~the robot has to step onto a relatively high platform to reach the goal for which it has to lean a little bit more to its right side to be able to lift the left foot further up.
  }%
